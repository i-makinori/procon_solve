...
(defun vdr-to-piece16- (vdr-list)
  (make-piece16- :vectors (vdr-vec-origin-shift (mapcar #'vdr-vec vdr-list))
              :degrees (mapcar #'vdr-deg vdr-list)))

(defun synthesize-piece16--rule (piece16-1 piece16-2)
    (let* ((vdr-queue
            (vdr-to-vdr-queue
             (piece16--to-vdr piece16-1)
             (piece16--to-vdr piece16-2)))
...
(defun piece16--to-vdr (piece16-)
  (mapcar #'(lambda (vec deg) (make-vdr :vec vec :deg deg))
(piece16--vectors piece16-) (piece16--degrees piece16-)))
