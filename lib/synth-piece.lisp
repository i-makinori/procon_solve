...
(defun remove-line (vdr-list)
  (remove-if
   #'(lambda (vdr) (a-d= pi (vdr-deg vdr)))
   vdr-list)
  )

(defun remove-round (vdr-list)
  (remove-if #'(lambda (vdr) (a-d= *2pi* (vdr-deg vdr))) vdr-list))
...
(defun synthesize-vdr (vdr1 vdr2)
  (if (and vdr1 vdr2
           (equal (vdr-vec vdr1) (vdr-vec vdr2)))
      (make-vdr :vec (vdr-vec vdr1)
...
(defun add-ridden-line-point-adjust (vdr-list &optional (s-vdr-list '()))
  (cond ((null (cdr vdr-list)) s-vdr-list)
        ((or (null s-vdr-list)
             (not (equal (vdr-vec (car vdr-list)) (vdr-vec (cadr vdr-list)))))
              ...
              (add-ridden-line-point
               ...
               (append
                (mapcar #'(lambda (p)
                        ...
                        (remove-if-not
                         #'(lambda (p)
                             (and (not (equal (vdr-vec p) this-point-vec))
                                  (not (equal (vdr-vec p) next-point-vec))
                                  (line-point-vec-hit-judge
